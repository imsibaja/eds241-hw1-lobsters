...
**Assignment submission (YOUR NAME):** Ian Morris-Sibaja
...
```{r}
# plot 2: Ridgeline Plot of Spiny Lobster Count Distribution by MPA Status

spiny_counts %>% 
    ggplot(aes(x = counts, y = mpa, fill = after_stat(quantile))) +
  stat_density_ridges(quantile_lines = TRUE,
                      calc_ecdf = TRUE,
                      geom = "density_ridges_gradient") +
  scale_fill_brewer(name = "Quantiles") +
    labs( x = "Total Counts of Spiny Lobster",
          y = "MPA Status",
          title = "Distribution of Spiny Lobster Counts by MPA Status") +
    theme_nice()
```
...
spiny_counts %>% 
    ggplot(aes(x = mean_size, y = mpa, fill = after_stat(quantile))) +
  stat_density_ridges(quantile_lines = TRUE,
                      calc_ecdf = TRUE,
                      geom = "density_ridges_gradient") +
  scale_fill_brewer(name = "Quantiles") +
...
Dispersion refers to the relationship between the variance of the observed data and the mean. In a perfectly fitting Poisson model, the dispersion is exactly 1, meaning the variance equals the mean.

Overdispersion occurs when the variance is greater than the mean. This means our model underestimates the variability of the data due to possible missing variables, dependent and clustering counts, or a an overabundance of zero counts in the data.
